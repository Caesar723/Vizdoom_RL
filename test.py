...
def get_state(game,num_objects):
    number=num_objects
    ...
    for i in range(6):#while True:
        ...
        while not game.is_episode_finished():
            ...
            if len(state_list) == agent.warmup_steps:
                ...
                with torch.no_grad():
                    if first_step:
                        torch_state_list = torch.tensor(state_list, dtype=torch.float32).unsqueeze(0).detach()
                        torch_obj_ids_list = torch.tensor(obj_ids_list, dtype=torch.int64).unsqueeze(0).detach()
                        torch_image1_list = torch.tensor(image1_list, dtype=torch.float32).unsqueeze(0).unsqueeze(2).detach()
                        torch_image2_list = torch.tensor(image2_list, dtype=torch.float32).unsqueeze(0).unsqueeze(2).detach()
                ...
                new_game_vars, new_obj_ids, new_normalized_depth, new_normalized_small_map = get_state(game,num_objects)
                ...
                next_torch_state_list = torch.tensor(state_list, dtype=torch.float32).unsqueeze(0).detach()
                next_torch_image1_list = torch.tensor(image1_list, dtype=torch.float32).unsqueeze(0).unsqueeze(2).detach()
                next_torch_image2_list = torch.tensor(image2_list, dtype=torch.float32).unsqueeze(0).unsqueeze(2).detach()
                next_torch_obj_ids_list = torch.tensor(obj_ids_list, dtype=torch.int64).unsqueeze(0).detach()
                if not first_step:
                    with torch.no_grad():
                        agent.store(
                            torch_state_list.squeeze(0), 
                ...
                game_vars, obj_ids, normalized_depth, normalized_small_map = get_state(game,num_objects)
