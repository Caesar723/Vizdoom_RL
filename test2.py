...
while True:
    ...
    while not game.is_episode_finished():
        ...
        with torch.no_grad():
            if first_step:
                torch_state_list = torch.tensor(game_vars, dtype=torch.float32).unsqueeze(0)
                torch_obj_ids_list = torch.tensor(obj_ids, dtype=torch.int64).unsqueeze(0)
                torch_image1_list = torch.tensor(normalized_depth, dtype=torch.float32).unsqueeze(0).unsqueeze(2)
                torch_image2_list = torch.tensor(normalized_small_map, dtype=torch.float32).unsqueeze(0).unsqueeze(2)
        ...
        next_torch_state_list = torch.tensor(new_game_vars, dtype=torch.float32).unsqueeze(0)
        next_torch_image1_list = torch.tensor(new_normalized_depth, dtype=torch.float32).unsqueeze(0).unsqueeze(2)
        next_torch_image2_list = torch.tensor(new_normalized_small_map, dtype=torch.float32).unsqueeze(0).unsqueeze(2)
        next_torch_obj_ids_list = torch.tensor(new_obj_ids, dtype=torch.int64).unsqueeze(0)
        if not first_step:
            with torch.no_grad():
                agent.store(
                    torch_state_list.squeeze(0), 
